...
const fillForm = async (isRegistered) => {
  ...
  const uniqueEmail = isRegistered ? 'customer@practicesoftwaretesting.com' : generateUniqueEmail();
...
When('I fill in an email that is already registered and submit the form', async () => {
  await fillForm(true);
});
